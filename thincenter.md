...
cmd_apply_model() {
  ...
  case "${primary_provider}" in
    nvidia)
      ...
      # Ensure the model ID is in the format expected by NVIDIA Integrate.
      # Most users should set PRIMARY_MODEL=nvidia/moonshotai/kimi-k2.5.
      local model_id="${primary_id}"
      if [[ "${model_id}" != moonshotai/* ]]; then
        model_id="moonshotai/${model_id}"
      fi

      local nvidia_cfg
      nvidia_cfg=$(cat <<EOF
{"baseUrl":"https://integrate.api.nvidia.com/v1","api":"openai-completions","apiKey":"${MOONSHOT_API_KEY}","models":[{"id":"${model_id}","name":"${model_id}","reasoning":true,"input":["text"],"cost":{"input":0,"output":0,"cacheRead":0,"cacheWrite":0},"contextWindow":${PRIMARY_MODEL_CONTEXT_WINDOW},"maxTokens":${PRIMARY_MODEL_MAX_TOKENS}}]}
EOF
)

      compose run --rm openclaw-cli \
        config set models.providers.nvidia "${nvidia_cfg}" --strict-json
      ;;
    ...
    moonshot)
      ...
      local moon_cfg
      moon_cfg=$(cat <<EOF
{"baseUrl":"https://api.moonshot.ai/v1","api":"openai-completions","apiKey":"${MOONSHOT_API_KEY}","models":[{"id":"${primary_id}","name":"${primary_id}","reasoning":false,"input":["text"],"cost":{"input":0,"output":0,"cacheRead":0,"cacheWrite":0},"contextWindow":${PRIMARY_MODEL_CONTEXT_WINDOW},"maxTokens":${PRIMARY_MODEL_MAX_TOKENS}}]}
EOF
)

      compose run --rm openclaw-cli \
        config set models.providers.moonshot "${moon_cfg}" --strict-json
      ;;

    *)
      fail "Unsupported PRIMARY_MODEL provider: ${primary_provider}. Use nvidia or moonshot."
      ;;
  esac
  ...
  # If validation fails, dump the provider section for inspection.
  if ! compose run --rm openclaw-cli config validate --json; then
    echo "" >&2
    echo "ERROR: OpenClaw config validation failed." >&2
    echo "Dumping models.providers for inspection:" >&2
    compose run --rm openclaw-cli config get models.providers --json || true
